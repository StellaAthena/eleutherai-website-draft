...
\big \| \mu_i' - \mu_i \big \|^2_{\mathcal H} + \big \| \tilde{\mathrm X}_i' - \tilde{\mathrm X}_i \big \|^2_{\mathcal H}.  \tag{2}
$$
...
**Proof.**
Let $\mathbb P(\mathrm Z_j)$ be the probability that the $j^{\text{th}}$ entry of $\mathrm Z$ is 1. Then for column $j$ of $\mathbf{\Sigma}_{XZ}$ we have
...
$$
\begin{equation}
    \mathbb{E}[\mathrm X] = (1 - \mathbb P(\mathrm Z_j)) \mathbb{E}[\mathrm X | \mathrm Z_j = 0] + \mathbb P(\mathrm Z_j) \mathbb{E}[\mathrm X | \mathrm Z_j = 1]. \tag{7}
\end{equation}
$$
