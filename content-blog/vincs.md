...
$$
\begin{equation}
    \mathbf{w^*} = \mathrm{TPC}(\{\mathcal{M}(s_i^+) - \mathcal{M}(s_i^-)\}) \tag{1}
\end{equation}
$$
...
$$
\begin{equation}
    \boldsymbol{\bar x}_{i}^+ = \frac{1}{k} \sum_{j=1}^k \mathcal{M}({s}_{ij}^+), \quad \boldsymbol{\bar x}_{i}^- = \frac{1}{k} \sum_{j=1}^k \mathcal{M}({s}^-_{ij}) \tag{6}
\end{equation}
$$
...
\begin{equation}
\begin{aligned}
    A_{\mathrm{confidence}} &= \mathrm{Cov}(\bar X^{+}) + \mathrm{Cov}(\bar X^{-})\\
    A_{\mathrm{consistency}} &= -\mathrm{Cov}(\bar X^{+}, \bar X^{-}) - \mathrm{Cov}(\bar X^{-}, \bar X^{+}).
\end{aligned}
\end{equation}
$$
